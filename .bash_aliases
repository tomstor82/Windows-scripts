...
alias pgo='ping -O 8.8.8.8';
alias p20='ping -O 192.168.1.20';
alias p88='ping -O 192.168.88.1';
# List shorthand
alias l='ls -1aF --color';
# Set IP
alias setip='/mnt/c/aliases/setIp';
# Display Local IP
alias lip='ifconfig | grep -Po "inet\s((\d){1,3}\.){3}(\d){1,3}" | grep -Po "\d+.+"';
# Display Public IP
...
# set IP shortcuts
alias dhcp='${netsh} interface ip set address "USB Ethernet" dhcp && echo Dynamic IP set && sleep 2 && pgo';
